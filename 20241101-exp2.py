        ...
        m := int(
            input(
                "\n".join(
                    [
                        "*" * 20,
                        str((items := {1: "电脑", 2: "鼠标", 3: "台灯", 4: "自行车"})),
                        "*" * 20,
                        "",
                        "请输入你想要购买的商品序号（按数字0结束）：",
                    ]
                )
            )
        )
...
def question10():
    ...
    print(
        f"The Plaintext is: {''.join([str(hashlist.index(c)) if c in hashlist else '?' for c in input('Please input the ciphertext(\'q\' for Exit):')])}"
    )
            ...
            [
                ...
                for x in list(
                    str(
                        sum(
                            [
                                sum([int(p) for p in str(int(n) * 2)])
                                for n in cardNums[:-1:2]
                            ]
                        )
                        + sum([int(n) for n in cardNums[1:-1:2]])
                    )
                )
            ]
        )
