...
def question1():
    ...
    def add_callback():
        textbox.insert(tk.END,entry1.get()+"+"+entry2.get()+"="+str(float(entry1.get())+float(entry2.get()))+"\n")
...
def question2():
    ...
    def calculateValue(*args):
        if radiovar.get()=="加上":
            ...
            return
        textbox.config(text=f"{entry1.get()}{radiovar.get()}{entry2.get()}等于{result:.2f}\n")
...
def question4():
    ...
    pearlradiobtn=tk.Radiobutton(rw,text="东方明珠",variable=posvar,value="东方明珠")
    ...
    priceMap={"东方明珠":160,"动物园":130,"科技馆":60}
    def calculatePrice():
